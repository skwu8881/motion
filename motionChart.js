...
var xScale = d3.scale.linear().domain([0, 150]).range([0, width]),
    yScale = d3.scale.linear().domain([0, 150]).range([height, 0]);
...
d3.json("data883.json", function(error, json) {
...
function draw(nations) {
    ...
    var dot = svg.selectAll(".dot")
        ...
        tooltip.html("<strong>城市:</strong>台南市 "+ "<br><br><strong>行政區:</strong> " + d.country + "<br><br><strong>登革熱病例數:</strong> " + d.gdp.toLocaleString());
    ...
    function position(dot) {
        dot.attr("cx", function (d) { return xScale(d.gdp/10); })
            .attr("cy", function (d) { return yScale(d.ecc/10); })
            .attr("r", function(d) { return d.ecc/20; })
